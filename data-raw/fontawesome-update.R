...
csv_content_fa_tbl <- stringi::stri_escape_unicode(csv_content_fa_tbl)
...
csv_content_fa_tbl <- as.character(strsplit(csv_content_fa_tbl, "\n"))
csv_content_fa_tbl <- paste0('  "', csv_content_fa_tbl, '",')
# Remove last trailing comma
csv_content_fa_tbl[length(csv_content_fa_tbl)] <-
  sub(",$", "", csv_content_fa_tbl[length(csv_content_fa_tbl)])


# Generate the code in R/fa_tbl.R
cat(
'# Generated by fontawesome-update.R; do not edit by hand\n
csv_content_fa_tbl <- c(\n',
  file = "R/fa_tbl.R"
)
cat(csv_content_fa_tbl, file = "R/fa_tbl.R", sep = "\n", append = TRUE)
cat(
')
tc <- textConnection(csv_content_fa_tbl)
fa_tbl <- read.table(tc, header = TRUE, quote = "", sep = ";", stringsAsFactors = FALSE)
rm(csv_content_fa_tbl)
close(tc)
rm(tc)
',
  file = "R/fa_tbl.R",
